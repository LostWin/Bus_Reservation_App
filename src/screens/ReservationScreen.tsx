...
const ReservationScreen = ({ navigation, route }) => {
  ...
  const posterImage = route.params?.PosterImage || 'default_image_url'; // Remplacez par une URL d'image par défaut

  useEffect(() => {
    console.log('Route params:', route.params); // Pour vérifier les paramètres passés
  }, [route.params]);
  ...
  const BookSeats = async () => {
    if (
      ...
      try {
        ...
        const response = await axios.post('http://10.0.2.2:3000/reservations/book', {
          userId,
          seats: selectedSeatArray,
          time: timeArray[selectedTimeIndex],
          date: dateArray[selectedDateIndex].date,
        });
        ...
        console.error('Something went wrong while booking seats:', error);
        ToastAndroid.showWithGravity(
          'Failed to book seats. Please try again.',
          ToastAndroid.SHORT,
          ToastAndroid.BOTTOM,
